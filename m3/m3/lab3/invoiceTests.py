...
def test_invoice_add_item_appends_item() -> tuple[bool, str]:
    ...
    invoice.add_item("Item 1", 1, 1.0, 0.2)
    # if (invoice.items[0].name != item.name or invoice.items[0].quantity != item.quantity or invoice.items[0].unit_price != item.unit_price or invoice.items[0].discount_percent != item.discount_percent):
    #     fail_case = True
    # else:
    #     fail_case = False
    fail_case = invoice.items[0].name != item.name or invoice.items[0].quantity != item.quantity or invoice.items[0].unit_price != item.unit_price or invoice.items[0].discount_percent != item.discount_percent
    expected = f"{item.name} {item.quantity} {item.unit_price} {item.discount_percent}" 
    actual = f"{invoice.items[0].name} {invoice.items[0].quantity} {invoice.items[0].unit_price} {invoice.items[0].discount_percent}"
