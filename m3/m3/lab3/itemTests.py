...
def test_calculate_item_total_price_converts_discount(expected_discount: float = 0.2) -> tuple[bool, str]:
    ...
    actual_discount = 1.0 - (item.calculate_item_total_price() / (item.unit_price * item.quantity))
    fail_case = round(actual_discount, 2) != round(expected_discount, 2)
...
def test_calculate_item_total_price_correct() -> tuple[bool, str]:
    ...
    expected_item_price = item.quantity* item.unit_price * (1 - item.discount_percent)
    actual_item_price = item.calculate_item_total_price()
    fail_case = round(actual_item_price, 2) != round(expected_item_price, 2)
